...
'Portb = &B00000000                                'исходное состояние линий B: 00000000
...
Const _usb_maxpower = 250                         '250 * 2mA = 500mA
...
Do
   ...
   If _usb_status._usb_rxc = 1 Then
      ...
      Elseif _usb_status._usb_endp1 = 1 Then      ' Пришёл пакет данных с компа
                  ...
                  If _usb_rx_buffer(2) = 255 Then 'Пришёл маркер - нулевой пакет. Только он может быть 255
                      Paket_dat = 0
                  Else
                      Incr Paket_dat
                  End If
    ...
    If Paket_dat = 1 Then
    Pwm0a = _usb_rx_buffer(2)                     'Данные 1 ШИМ могут быть 0-254
    End If

    If Paket_dat = 2 Then
    Pwm0b = _usb_rx_buffer(2)                     'Данные 2 ШИМ могут быть 0-254
    End If

    If Paket_dat = 3 Then
    Pwm1a = _usb_rx_buffer(2)                     'Данные 3 ШИМ могут быть 0-254
    End If

    If Paket_dat = 4 Then
    Pwm1b = _usb_rx_buffer(2)                     'Данные 4 ШИМ могут быть 0-254
    End If

    If Paket_dat = 5 Then
    Pwm2a = _usb_rx_buffer(2)                     'Данные 5 ШИМ могут быть 0-254
    End If

    If Paket_dat = 6 Then
    Pwm2b = _usb_rx_buffer(2)                     'Данные 6 ШИМ могут быть 0-254
    End If
...
_usb_proddescriptor:
Data 28 , 28 , _usb_desc_string
Data "R" , "a" , "d" , "a" , "n" , " " , "L" , "E" , "D" , " "
Data "v" , "1" , "." , "0"
