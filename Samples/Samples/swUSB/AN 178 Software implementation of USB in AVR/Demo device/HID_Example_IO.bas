...
Declare Sub Usb_processsetup(byref Txstate() As Byte)
Declare Sub Usb_send(byref Txstate() As Byte , Byval Count As Byte)
Declare Sub Usb_senddescriptor(byref Txstate() As Byte , Maxlen As Byte)
...
Const _usb_dminus = 3                             '5

Dim Swusb_resetcounter As Word
Dim Swusb_idlemode As Byte
...
'  inserted by six1 2010/07
Const Caps_outputreportbytelength = 5
Const Caps_inputreportbytelength = 4
...
Do
   ...
   If _usb_status._usb_rxc = 1 Then
      If _usb_status._usb_setup = 1 Then
         ...
         Call Usb_processsetup(_usb_tx_rawbuffer(1))
   ...
   'If Buttons_current <> Buttons_last Then
    'Queue data to be sent on endpoint 2 (HID report)
     If _usb_tx_status2(1)._usb_txc = 1 Then
...
' six1 fix array 2012-11-28
Sub Usb_senddescriptor(byref Txstate() As Byte , Maxlen As Byte)
...
' six1 fix array 2012-11-28
         Read Txstate(i)
'         Read _usb_tx_rawbuffer(i)
      #endif

      #if Usbdebug = 1
' six1 fix array 2012-11-28
         Print Bin(txstate(i))
'         Print Bin(_usb_tx_rawbuffer(i))
      #endif
...
Sub Usb_send(byref Txstate() As Byte , Byval Count As Byte)
#if Usbdebug = 2
Dbg
#endif
...
' OLD LIB!!!
' six1 fix array 2012-11-28
'      _usb_crc = Crc16uni(_usb_tx_rawbuffer(3) , Count , &HFFFF , &H8005 , 1 , 1)
...
' six1 fix array 2012-11-28
'   _usb_tx_rawbuffer(count) = Low(_usb_crc)
'   Incr Count
'   _usb_tx_rawbuffer(count) = High(_usb_crc)
...
' six1 fix array 2012-11-28
'   _usb_tx_rawbuffer(2)  = _usb_pid_data1
'   If Txstate._usb_txsync = 0 Then
'   _usb_tx_rawbuffer(2) = _usb_pid_data0
'   End If
...
' six1 fix array 2012-11-28
   Set Txstate(1)._usb_rtt
   Reset Txstate(1)._usb_txc
End Sub
